...
- title - just the title of the page as it would appear on html (!NOTETODOTODO MIGHT MOVE THIS FILE)
- permalink - the route that jekyll will use in this repo (export the page might change (!NOTETODOTODO ADD LINK TO EXPORT README HERE))
- data_file - the file in \_data folder to get all information for the page (!NOTETODOTODO ADD LINK TO DATA FILES HERE)
...
- favicon - boolean to use favicons or not (!NOTETODOTODO CHANGE THIS WHEN ADD OPTION TO USE OWN FAVICON)
...
an example of this
  ...
  (!NOTETODOTODO LINK TO COFFEE, SASS, NEW PARTIALS, AND DATA FILES HERE)
